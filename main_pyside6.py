...
class TodoListApp(QtWidgets.QDialog):
    ...
    def showOrUpdateCommentDialog(self, item):
        ...
        if self.comment_dialog and self.comment_dialog.isVisible():
            if index == self.comment_dialog.current_index:
                self.comment_dialog.comment.setText(comment)
                return
            self.comment_dialog.close()
    ...
    def renameTodoItem(self, item):
        ...
        new_task, ok = QtWidgets.QInputDialog.getText(
            self, 'Rename Task', 'Enter new task name:', text=current_task
        )
    ...
    def populateTodoList(self):
        ...
        for todo in self.todo_data:
            ...
            item.setCheckState(
                QtCore.Qt.Checked if todo.get('completed') else QtCore.Qt.Unchecked
            )
...
class CommentDialog(QtWidgets.QDialog):
    ...
    def accept(self):
        super().accept()
        todo_app.updateComment(self.current_index, self.comment.toPlainText())
